...
/*
DPCT1110:2: The total declared local variable size in device function rms_norm exceeds 128 bytes and
...
#pragma unroll
    ...
    reduce::partitioned_block<rop::Add, threadsPerGroup>(
        tb, warp, var_sum, item_ct1, reduce_buffer);
    const float var = var_sum / elems_per_row;
    /*
    DPCT1013:6: The rounding mode could not be specified and the generated code may have different
...
template <typename T, int UNROLL, int threadsPerGroup, int maxThreads>
/*
DPCT1110:3: The total declared local variable size in device function pre_rms_norm exceeds 128 bytes
...
#pragma unroll
    ...
    const float var = var_sum / elems_per_row;
    /*
    DPCT1013:7: The rounding mode could not be specified and the generated code may have different
    accuracy than the original code. Verify the correctness. SYCL math built-in function rounding
    mode is aligned with OpenCL C 1.2 standard.
    */
    const T denom = conversion::to<T>(sycl::rsqrt(var + epsilon));
...
#define LAUNCH_RMS_NORM(UNROLL, threadsPerGroup, maxThreads)                                      \
  ...
  stream->submit([&](sycl::handler& cgh) {                                                        \
    sycl::local_accessor<float, 1> reduce_buffer_acc_ct1(sycl::range<1>(hw_warp_size * 2), cgh); \
                                                                                                  \
...
#define LAUNCH_PRE_RMS_NORM(UNROLL, threadsPerGroup, maxThreads)                                  \
  ...
  stream->submit([&](sycl::handler& cgh) {                                                        \
    ...
    cgh.parallel_for(sycl::nd_range<3>(grid * block, block),                                      \
                     [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {          \
                       pre_rms_norm<T, UNROLL, threadsPerGroup, maxThreads>(                      \
                           norm_output_ct0,                                                       \
                           res_output_ct1,                                                        \
                           vals_ct2,                                                              \
                           residual_ct3,                                                          \
