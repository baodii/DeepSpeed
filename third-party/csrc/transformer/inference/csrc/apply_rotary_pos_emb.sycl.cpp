                           ...
                           const sycl::nd_item<3>& item_ct1)
{
    ...
    auto head_group = item_ct1.get_sub_group();
    ...
    if (head_idx < total_count) {
        ...
        */
        const int base_neuron_idx = head_group.get_local_id() * T_per_thread;
...
#pragma unroll
        for (int i = 0; i < T_per_thread; i++) {
            ...
            if (neuron_idx < rotary_dim) {
                ...
                inv_freq = 1.0 / sycl::pow<float>(10000.0, inv_freq) * (float)seq_idx;
                ...
                const int target_lane = (neuron_idx < half_dim)
                                            ...
                                            */
                                            ? head_group.get_local_id() + half_dim_threads
                                            /*
...
#define LAUNCH_ROT_POS_EMB_HALF(HEAD_THREADS, ALIGNMENT)                                          \
  {                                                                                               \
    ...
    stream->submit([&](sycl::handler& cgh) {                                                      \
      ...
      auto max_out_tokens_ct8 = max_out_tokens;                                                   \
                                                                                                  \
...
#define INSTANTIATE_LAUNCH_ROTARY_POS_EMB(T)      \
    template void launch_apply_rotary_pos_emb<T>( \
        T*, T*, unsigned, unsigned, unsigned, unsigned, unsigned, unsigned, dpct::queue_ptr, int);
